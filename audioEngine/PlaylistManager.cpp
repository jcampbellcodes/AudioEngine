...
std::list<PlaylistInitializationCommand*> ParseScript(const char* path_to_script);
...
snd_err PlaylistManager::privInitializePlaylistDescriptor(const char * path_to_script, SoundCallID id)
{
	snd_err err = snd_err::OK;

	//loading audio needs to be async... with a callback
	// load sounds -----------
	std::list<PlaylistInitializationCommand*> cmds = ParseScript(path_to_script);
...
std::list<PlaylistInitializationCommand*> ParseScript(const char* path_to_script)
{
	...
	while (delineatorEnd != soundList.npos) // does this condition work?
	{
		...
		if (path.size() > 0)
		{
			newCommand = new PlaylistAddSoundCommand(key, path.c_str());
...
snd_err PlaylistManager::privStartPlaylist(SoundCallID id, unsigned int handle)
{
	...
	while (iter != playlistDescriptors.end())
	{
		if ((*iter)->playlistID == id)
		{
			...
			(*iter)->factory->CreatePlaylist(playlist, handle);
			assert(playlist);
